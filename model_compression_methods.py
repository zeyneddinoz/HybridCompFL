...
def quantize_zero_aware(tensor, num_bits=8, min_nonzero=0.01):
    ...
    # Set small values to zero for zero-awareness
    non_zero_mask = q_tensor.abs() > min_nonzero
    q_tensor[~non_zero_mask] = 0
    ...
    scale = max_val / 127.0  # Symmetric for qint8 (-128 to 127, but use 127 for safety)
...
def apply_zero_aware_quantization(model):
    ...
    with torch.no_grad():
        for name, module in quantized_model.named_modules():  # Loop over modules
            if isinstance(module, (nn.Conv2d, nn.Linear)) and hasattr(module, 'weight'):
                q_data = quantize_zero_aware(module.weight.data)
                module.weight = nn.Parameter(q_data, requires_grad=False)  # Assign new Parameter without grad
