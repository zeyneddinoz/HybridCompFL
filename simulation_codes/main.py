...
GLOBAL_ROUND = 100                   # 100
TOTAL_DEVICES_IN_THE_SYSTEM = 100    # 100, 50
PARTICIPATION_RATE = 0.6            # (60% of randomly selected devices share their LM with the server per global round.)
PERCENTAGE_OF_CAPABLE_DEVICES = 0.2 # For 0.2, only 20% of devices can train GM.
LOCAL_EPOCH = 5                     # 3, 5
...
# In[3] ################################### FL TRAINING ###################################
# === Train model once ===
trained_model, test_loader, data_dist = experiment_setting(
    model_name=MODEL_NAME,
    dataset_name=DATASET_NAME,
    aggregation_method=AGGREGATION_METHOD,
