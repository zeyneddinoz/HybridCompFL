...
The left figure above shows that the server requires all data to train a central model (CM) in this system. For instance, device $d_1$ shares its datasets to participate to the system, so that the server can train a central model and then deploy it to device $d_1$. This process is the same for all devices in $D = \{d_1, d_2, \dots, d_n\}$. 
...
The hyperparameters utilized in this work are listed below:
