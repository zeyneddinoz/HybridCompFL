...
for AMOUNT in range(1, 10, 1):
    ...
    AMOUNT_percent = int(AMOUNT_val * 100)
    ...
    sm_filename = f"sm_then_qm_sparsed_model_{AMOUNT_percent}.pth"
    ...
    # Save quantized
    qm_filename = f"sm_then_qm_quantized_model_{AMOUNT_percent}.pth"
    torch.save(quantized_model.state_dict(), qm_filename)
    qm_size = os.path.getsize(qm_filename) / (1024**2)
    
    # Store model paths in results
    if f"amount_{AMOUNT_percent}" not in results["saved_models"]:
        results["saved_models"][f"amount_{AMOUNT_percent}"] = {}
    results["saved_models"][f"amount_{AMOUNT_percent}"]["sm_then_qm_sparsed"] = sm_filename
    results["saved_models"][f"amount_{AMOUNT_percent}"]["sm_then_qm_quantized"] = qm_filename
...
for AMOUNT in range(1, 10, 1):
    AMOUNT_val = AMOUNT/10
    AMOUNT_percent = int(AMOUNT_val * 100)  # Convert to percentage
    ...
    sparse_sm = make_sparse(sparsified_model)
    sm_filename = f"qm_then_sm_sparsed_model_{AMOUNT_percent}.pth"
    torch.save(sparse_sm.state_dict(), sm_filename)
    sm_size = os.path.getsize(sm_filename) / (1024**2)
    ...
    if f"amount_{AMOUNT_percent}" not in results["saved_models"]:
        results["saved_models"][f"amount_{AMOUNT_percent}"] = {}
    results["saved_models"][f"amount_{AMOUNT_percent}"]["qm_then_sm_sparsed"] = sm_filename
